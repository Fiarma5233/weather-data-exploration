...
# #     # Récupération des fichiers et des stations associées
# #     uploaded_files = request.files.getlist('file[]')
# #     stations = [
...
# #     # Validation du nombre de fichiers vs stations
# #     if len(uploaded_files) != len(stations):
# #         flash('Nombre de fichiers et de stations incompatible', 'error')
# #         return redirect(url_for('index'))

# #     # Préparation du DataFrame pour ce batch
# #     batch_dfs = []
...
# #         else:
# #             # Suppression des anciennes données pour ces stations
...
# # # def statistiques():
# # #     """Génère les statistiques météorologiques pour une variable sélectionnée"""
    
# # #     # Vérification des données disponibles
# # #     if GLOBAL_PROCESSED_DATA_DF.empty:
# # #         flash("Aucune donnée disponible. Chargez d'abord des fichiers météo.", 'warning')
# # #         return redirect(url_for('visualizations_options'))

# # #     # Gestion de la requête POST (soumission du formulaire)
# # #     if request.method == 'POST':
# # #         variable = request.form.get('variable_meteo')
        
# # #         # Validation de la variable sélectionnée
# # #         if not variable or variable not in GLOBAL_PROCESSED_DATA_DF.columns:
# # #             flash("Sélectionnez une variable valide", 'error')
# # #             return redirect(url_for('statistiques'))

# # #         try:
# # #             # Génération du graphique
# # #             stats_fig = generate_stats_plots(GLOBAL_PROCESSED_DATA_DF, variable)
            
# # #             if not stats_fig:
# # #                 flash("Graphique non généré pour cette variable", 'error')
# # #                 return redirect(url_for('statistiques'))
            
# # #             # Sauvegarde temporaire de l'image
# # #             img_dir = os.path.join(app.static_folder, 'temp')
# # #             os.makedirs(img_dir, exist_ok=True)
            
# # #             img_path = os.path.join(img_dir, f'stats_{variable}.png')
# # #             stats_fig.savefig(img_path, bbox_inches='tight', dpi=100)
# # #             plt.close(stats_fig)
            
# # #             # Préparation des données pour le template
# # #             variable_meta = METADATA_VARIABLES.get(variable, {})
# # #             stats_data = {
# # #                 'variable': variable,
# # #                 'img_url': f'/static/temp/stats_{variable}.png',
# # #                 'variable_name': variable_meta.get('Nom', variable),
# # #                 'unit': variable_meta.get('Unite', '')
# # #             }
            
# # #             return render_template('statistiques.html', **stats_data)

# # #         except Exception as e:
# # #             app.logger.error(f"Erreur génération stats: {str(e)}")
...
# # #         (col, METADATA_VARIABLES.get(col, {}).get('Nom', col))
# # #         for col in GLOBAL_PROCESSED_DATA_DF.columns
# # #         if col not in excluded_cols and pd.api.types.is_numeric_dtype(GLOBAL_PROCESSED_DATA_DF[col])
# # #     ], key=lambda x: x[1])  # Tri par nom affiché

# # #     return render_template('statistiques.html', variables=available_vars)
...
# # # @app.route('/visualisations/statistiques', methods=['GET', 'POST'])
# # # def statistiques():
# # #     if 'processed_data' not in session:
# # #         flash("Aucune donnée disponible. Veuillez d'abord charger et traiter des données.", 'warning')
# # #         return redirect(url_for('visualizations_options'))
...
# # #             flash(f"La variable {variable} n'est pas disponible dans les données.", 'error')
# # #             return redirect(url_for('visualisations_options'))
        
# # #         # Générer les graphiques
# # #         stats_fig = generate_stats_plots(df, variable)
        
# # #         if not stats_fig:
# # #             flash("Impossible de générer les statistiques pour cette variable.", 'error')
# # #             return redirect(url_for('visualisations_options'))
            
# # #         # Sauvegarde temporaire de l'image
# # #         img_path = os.path.join('static', 'temp_stats.png')
# # #         stats_fig.savefig(img_path, bbox_inches='tight', dpi=100)
# # #         plt.close(stats_fig)
...
# # #     """Supprime les fichiers temporaires des graphiques"""
# # #     temp_files = ['temp_stats.png']  # Ajoutez d'autres noms si nécessaire
...
# #     if GLOBAL_PROCESSED_DATA_DF.empty:
# #         flash("Aucune donnée disponible. Chargez d'abord des fichiers météo.", 'warning')
# #         return redirect(url_for('visualizations_options'))  # Changé ici
...
# #             img_dir = os.path.join(app.config['STATIC_FOLDER'], 'temp')
# #             os.makedirs(img_dir, exist_ok=True)
...
# #         except Exception as e:
# #             flash(f"Erreur lors de la génération des statistiques: {str(e)}", 'error')
# #             return redirect(url_for('statistiques'))
...
# app = Flask(__name__)
# app.secret_key = 'votre_cle_secrete_ici'
# app.config['MAX_CONTENT_LENGTH'] = 64 * 1024 * 1024  # Correction de la typo CONTENT
# app.config['UPLOAD_FOLDER'] = 'uploads'
# app.config['STATIC_FOLDER'] = 'static'  # Ajout de la config manquante
...
#         if not variable or not periode:
#             flash('Veuillez sélectionner une variable et une période', 'error')
#             return redirect(url_for('visualisations_options'))  # Modifié ici
...
#         img_dir = os.path.join(app.static_folder, 'temp_stats')
#         os.makedirs(img_dir, exist_ok=True)
        
#         # Nom du fichier image
#         img_filename = f'stats_{secure_filename(variable)}.png'
...
#         fig = generate_daily_stats_plot(GLOBAL_PROCESSED_DATA_DF, variable)
...
#         fig.savefig(img_path, bbox_inches='tight', dpi=100, format='png')
#         plt.close(fig)

#         # Vérification que l'image a bien été créée
#         if not os.path.exists(img_path):
#             flash("L'image n'a pas pu être générée", 'error')
#             return redirect(url_for('visualisations_options'))
...
#                             img_url=url_for('static', filename=f'temp_stats/{img_filename}'))
...
# app = Flask(__name__)
# app.secret_key = 'votre_cle_secrete_ici' # Gardez votre clé secrète ici
# app.config['MAX_CONTENT_LENGTH'] = 64 * 1024 * 1024
# app.config['UPLOAD_FOLDER'] = 'uploads'
# app.config['STATIC_FOLDER'] = 'static'
...
#     uploaded_files = request.files.getlist('file[]')
#     stations = []
#     # Collecter les stations pour tous les fichiers
#     for i in range(len(uploaded_files)):
#         station_name = request.form.get(f'station_{i}')
#         if station_name:
#             stations.append(station_name)
#         else:
#             # Gérer le cas où une station n'est pas fournie pour un fichier
...
#             os.unlink(temp_path) # Supprime le fichier temporaire après lecture
...
#         # Gérer la mise à jour ou l'initialisation de GLOBAL_PROCESSED_DATA_DF
#         if GLOBAL_PROCESSED_DATA_DF.empty:
#             GLOBAL_PROCESSED_DATA_DF = batch_df
#         else:
#             # Supprimer les données existantes pour les stations qui sont mises à jour
#             stations_to_update = batch_df['Station'].unique()
#             GLOBAL_PROCESSED_DATA_DF = GLOBAL_PROCESSED_DATA_DF[
#                 ~GLOBAL_PROCESSED_DATA_DF['Station'].isin(stations_to_update)
#             ]
...
#         if not fig or not fig.data: # Vérifier si la figure est vide
...
#         # 2. Nettoyer le dossier d'upload si nécessaire (optionnel, car les fichiers sont déjà unlinked après upload)
#         # Ceci est plus une mesure de précaution si des fichiers temporaires restaient pour une raison ou une autre.
#         # Attention: ne supprimez que les fichiers que vous savez être temporaires ou liés à l'upload.
...
#                     os.unlink(file_path)
#             except Exception as e:
#                 app.logger.warning(f"Impossible de supprimer le fichier {file_path}: {e}")
#                 # Ne pas bloquer la réinitialisation si un fichier ne peut pas être supprimé

#         # 3. Flasher un message de succès (sera affiché après la redirection)
...
#         # 4. Indiquer au client de rediriger vers la page d'accueil
...
#         # Même en cas d'erreur côté serveur, tenter de rediriger l'utilisateur vers la page d'accueil
#         # pour éviter qu'il ne reste bloqué sur la page de visualisation sans données.
...
#         if not fig or not fig.data: # Vérifier si la figure est vide
#             flash('Aucune donnée disponible pour cette variable. Veuillez affiner votre sélection.', 'warning')
#             return redirect(url_for('visualisations_options'))
