...
#             total_rows = len(metrics_for_new_plots)
#             # AUGMENTATION DE LA HAUTEUR ET DE L'ESPACEMENT POUR PLUS D'AIR
#             row_height_per_plot = 400 # Augmenté à 400 (par ex.) pour laisser plus de place
#             vertical_spacing_for_new_plots = 0.12 # Augmenté à 0.12 pour plus d'espace entre les lignes de subplots
...
#                     # Assurer que les NaNs sont traités pour les tracés de barres
#                     y_vals = station_data[metric].fillna(0).values # Remplacer NaN par 0 pour le tracé
...
#                         # Utiliser la valeur non-fillna pour le hovertext si elle n'est pas NaN
...
#                 # Assurer que l'axe Y commence à 0 pour les barres
#                 fig_existing.update_yaxes(rangemode='tozero', row=row, col=col)
...
#             total_rows = len(metrics_for_new_plots)
#             # AUGMENTATION DE LA HAUTEUR ET DE L'ESPACEMENT POUR PLUS D'AIR
#             # J'ai encore augmenté la hauteur par plot pour donner plus d'espace
#             row_height_per_plot = 450 # Augmenté à 450 pour plus d'espace
#             vertical_spacing_for_new_plots = 0.15 # Augmenté à 0.15 pour plus d'espace entre les lignes de subplots
...
#             # Le titre principal est mis à jour APRÈS les axes pour éviter le chevauchement avec les row_titles
#             fig_new_stats.update_layout(height=row_height_per_plot * total_rows) # On met la hauteur ici, le titre après
...
#                 for station_name in unique_stations:
#                     station_mean_val = df_station_extreme_means[df_station_extreme_means['Station'] == station_name]['Overall_Mean'].iloc[0]
#                     # Pour les valeurs nulles, nous traçons 0 mais affichons "N/A" dans le hover
#                     display_mean_val = f"{station_mean_val:.2f}" if pd.notna(station_mean_val) else _("N/A")
#                     val_for_plot = station_mean_val if pd.notna(station_mean_val) else 0 # Tracage de 0 si NaN

#                     hover_text_mean = f"<b>{_('Station')}:</b> {station_name}<br><b>{metric_label}:</b> {display_mean_val} {unit}"
#                     if global_period_str: hover_text_mean += f"<br>{global_period_str}"
...
#                     # Assurer que l'axe Y commence à 0 pour les barres
#                     fig_new_stats.update_yaxes(rangemode='tozero', row=row_idx, col=1)
...
#                     val_for_plot = station_max_val if pd.notna(station_max_val) else 0 # Tracage de 0 si NaN

#                     hover_text_max = f"<b>{_('Station')}:</b> {station_name}<br><b>{metric_label}:</b> {display_max_val} {unit}"
#                     if pd.notna(station_max_period): hover_text_max += f"<br><b>{_('Année')}:</b> {int(station_max_period)}"
#                     if pd.notna(overall_mean_for_station):
#                         hover_text_max += f"<br>({_('Moyenne sur la période des données')}: {overall_mean_for_station:.2f} {unit})"
#                     if global_period_str: hover_text_max += f"<br>{global_period_str}"
...
#                     # Assurer que l'axe Y commence à 0 pour les barres
#                     fig_new_stats.update_yaxes(rangemode='tozero', row=row_idx, col=2)
...
#                     val_for_plot = station_min_val if pd.notna(station_min_val) else 0 # Tracage de 0 si NaN

#                     hover_text_min = f"<b>{_('Station')}:</b> {station_name}<br><b>{metric_label}:</b> {display_min_val} {unit}"
#                     if pd.notna(station_min_period): hover_text_min += f"<br><b>{_('Année')}:</b> {int(station_min_period)}"
#                     if pd.notna(overall_mean_for_station):
#                         hover_text_min += f"<br>({_('Moyenne sur la période des données')}: {overall_mean_for_station:.2f} {unit})"
#                     if global_period_str: hover_text_min += f"<br>{global_period_str}"
...
#                     # Assurer que l'axe Y commence à 0 pour les barres
#                     fig_new_stats.update_yaxes(rangemode='tozero', row=row_idx, col=3)
...
#             # Mettre à jour le titre du layout APRÈS avoir défini les axes et les row_titles
#             fig_new_stats.update_layout(
#                 title_text=_("Statistiques Récapitulatives Annuelles de {} par Station").format(var_label),
#                 showlegend=True,
#                 legend_title_text=_("Station"),
...
#                 display_mean_val = f"{station_mean_val:.2f}" if pd.notna(station_mean_val) else _("N/A")
#                 val_for_plot = station_mean_val if pd.notna(station_mean_val) else 0 # Plot 0 if NaN
...
#                 # Assurer que l'axe Y commence à 0 pour les barres
#                 fig_generic.update_yaxes(rangemode='tozero', row=1, col=1)
...
#                 display_max_val = f"{station_max_val:.2f}" if pd.notna(station_max_val) else _("N/A")
#                 val_for_plot = station_max_val if pd.notna(station_max_val) else 0 # Plot 0 if NaN
...
#                 # Assurer que l'axe Y commence à 0 pour les barres
#                 fig_generic.update_yaxes(rangemode='tozero', row=1, col=2)
...
#                 display_min_val = f"{station_min_val:.2f}" if pd.notna(station_min_val) else _("N/A")
#                 val_for_plot = station_min_val if pd.notna(station_min_val) else 0 # Plot 0 if NaN
...
#                 # Assurer que l'axe Y commence à 0 pour les barres
#                 fig_generic.update_yaxes(rangemode='tozero', row=1, col=3)
...
#             total_rows = len(metrics_for_new_plots)
#             # Revertir la hauteur par plot et l'espacement à ce qui était proche de l'original ou par défaut
#             row_height_per_plot = 400 # Reverti à 400
#             vertical_spacing_for_new_plots = 0.08 # Reverti à 0.08
...
#                 plot_bgcolor='white',
#                 paper_bgcolor='white',
#                 # Revertir la marge supérieure à la valeur proche de l'original
#                 margin=dict(b=150, t=100) # Reverti t=150 à t=100
#             )
...
#             total_rows = len(metrics_for_new_plots)
#             # Augmenter la hauteur par plot et l'espacement
#             row_height_per_plot = 500  # Augmenté de 400 à 500
#             vertical_spacing_for_new_plots = 0.15  # Augmenté de 0.08 à 0.15
...
#                 # Colonne 1 - Garder le titre avec unité
#                 fig_new_stats.update_yaxes(
#                     title_text=f"{metric_label} ({unit})", 
#                     row=row_idx, 
#                     col=1
#                 )
                
#                 # Colonne 2 - Garder le titre avec unité
#                 fig_new_stats.update_yaxes(
#                     title_text=f"{metric_label} ({unit})", 
#                     row=row_idx, 
#                     col=2
...
#                 # Colonne 3 - Supprimer le titre de l'axe Y
#                 fig_new_stats.update_yaxes(
#                     title_text="",  # Pas de titre pour la colonne 3
#                     row=row_idx, 
#                     col=3
#                 )
...
#                 # Filter for daylight hours using 'Is_Daylight' for Solar_R
#                 if variable == 'Solar_R_W/m^2':
#                     if 'Is_Daylight' in station_data_processed.columns:
#                         station_data_processed = station_data_processed[station_data_processed['Is_Daylight'] == True]
#                     else:
...
#             total_rows = len(metrics_for_new_plots)
#             # Augmenter la hauteur par plot et l'espacement vertical
#             row_height_per_plot = 500  # Augmenté de 400 à 500
#             vertical_spacing_for_new_plots = 0.15  # Augmenté de 0.08 à 0.15
...
#                 # Mise à jour des axes Y - seulement pour la colonne 1
#                 fig_new_stats.update_yaxes(title_text=f"{metric_label} ({unit})", row=row_idx, col=1)
#                 fig_new_stats.update_yaxes(title_text="", row=row_idx, col=2)  # Pas de titre pour la colonne 2
#                 fig_new_stats.update_yaxes(title_text="", row=row_idx, col=3)  # Pas de titre pour la colonne 3
...
#                 # Mettre à jour les axes Y seulement pour la première colonne
#                 fig_new_stats.update_yaxes(title_text=f"{metric_label} ({unit})", row=row_idx, col=1)
#                 fig_new_stats.update_yaxes(title_text="", row=row_idx, col=2)  # Supprimer le titre pour la colonne 2
#                 fig_new_stats.update_yaxes(title_text="", row=row_idx, col=3)  # Supprimer le titre pour la colonne 3
...
#             metrics_to_plot_info_existing = [
#                 {'metric_col': 'Cumul Annuel', 'title': get_metric_label("Cumul Annuel des Précipitations"), 'row': 1, 'col': 1, 'unit': var_unit, 'yaxis_title': "Cumul Annuel des Précipitations (mm)"},
#                 {'metric_col': 'Moyenne Jours Pluvieux', 'title': get_metric_label("Précipitation Moyenne des Jours Pluvieux"), 'row': 1, 'col': 2, 'unit': var_unit, 'yaxis_title': "Précipitation Moyenne des Jours Pluvieux (mm)"},
#                 {'metric_col': 'Moyenne Saison Pluvieuse', 'title': get_metric_label("Précipitation Moyenne de la Saison Pluvieuse"), 'row': 2, 'col': 1, 'unit': var_unit, 'yaxis_title': "Précipitation Moyenne de la Saison Pluvieuse (mm)"},
#                 {'metric_col': 'Valeur Max Jours Pluvieux Annuelle', 'title': get_metric_label("Jour le plus pluvieux"), 'row': 2, 'col': 2, 'unit': var_unit, 'date_col': 'Date Max Pluvieux Annuelle'},
#                 {'metric_col': 'Valeur Min Jours Pluvieux Annuelle', 'title': get_metric_label("Jour le moins pluvieux"), 'row': 3, 'col': 1, 'unit': var_unit, 'date_col': 'Date Min Pluvieux Annuelle'},
#                 {'metric_col': 'Durée Saison Pluvieuse Jours', 'title': get_metric_label("Durée de la Saison Pluvieuse"), 'row': 3, 'col': 2, 'unit': get_metric_label("jours"), 'yaxis_title': "Durée de la Saison Pluvieuse (jours)"},
#                 {'metric_col': 'Durée Jours Sans Pluie', 'title': get_metric_label("Plus Longue Durée des Jours Sans Pluie"), 'row': 4, 'col': 1, 'unit': get_metric_label("jours"), 'yaxis_title': "Plus Longue Durée des Jours Sans Pluie (jours)"},
#                 {'metric_col': 'Durée Sécheresse Définie Jours', 'title': get_metric_label("Durée de la Sécheresse Définie"), 'row': 4, 'col': 2, 'unit': get_metric_label("jours"), 'yaxis_title': "Durée de la Sécheresse Définie (jours)"}
#             ]
...
#                 # Ajout du titre de l'axe Y seulement pour le premier graphique de chaque ligne
...
#             metrics_for_new_plots = [
#                 {'col': 'Cumul Annuel', 'unit': var_unit, 'label': "Cumul Annuel des Précipitations (mm)", 'yaxis_title': "Cumul Annuel des Précipitations (mm)"},
#                 {'col': 'Moyenne Jours Pluvieux', 'unit': var_unit, 'label': "Précipitation Moyenne des Jours Pluvieux (mm)", 'yaxis_title': "Précipitation Moyenne des Jours Pluvieux (mm)"},
#                 {'col': 'Moyenne Saison Pluvieuse', 'unit': var_unit, 'label': "Précipitation Moyenne de la Saison Pluvieuse (mm)", 'yaxis_title': "Précipitation Moyenne de la Saison Pluvieuse (mm)"},
#                 {'col': 'Durée Saison Pluvieuse Jours', 'unit': "jours", 'label': "Durée de la Saison Pluvieuse (jours)", 'yaxis_title': "Durée de la Saison Pluvieuse (jours)"},
#                 {'col': 'Durée Jours Sans Pluie', 'unit': "jours", 'label': "Plus Longue Durée des Jours Sans Pluie (jours)", 'yaxis_title': "Plus Longue Durée des Jours Sans Pluie (jours)"},
#                 {'col': 'Durée Sécheresse Définie Jours', 'unit': "jours", 'label': "Durée de la Sécheresse Définie (jours)", 'yaxis_title': "Durée de la Sécheresse Définie (jours)"}
#             ]
...
#                 # Ajout du titre de l'axe Y seulement pour la première colonne
...
#             metrics_for_new_plots = [
#                 {'col': 'Cumul Annuel', 'unit': "mm", 'label': "Cumul Annuel des Précipitations (mm)", 'yaxis_title': "Précipitation (mm)"},
#                 {'col': 'Moyenne Jours Pluvieux', 'unit': "mm", 'label': "Précipitation Moyenne des Jours Pluvieux (mm)", 'yaxis_title': "Précipitation (mm)"},
#                 {'col': 'Moyenne Saison Pluvieuse', 'unit': "mm", 'label': "Précipitation Moyenne de la Saison Pluvieuse (mm)", 'yaxis_title': "Précipitation (mm)"},
#                 {'col': 'Durée Saison Pluvieuse Jours', 'unit': "jours", 'label': "Durée de la Saison Pluvieuse (jours)", 'yaxis_title': "Jours"},
#                 {'col': 'Durée Jours Sans Pluie', 'unit': "jours", 'label': "Plus Longue Durée des Jours Sans Pluie (jours)", 'yaxis_title': "Jours"},
#                 {'col': 'Durée Sécheresse Définie Jours', 'unit': "jours", 'label': "Durée de la Sécheresse Définie (jours)", 'yaxis_title': "Jours"}
#             ]
